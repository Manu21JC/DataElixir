...
def add_args(parser):
    ...
    parser.add_argument('--yaml_path', type=str, default='../config/badnet/cifar10.yaml',
    ...
    parser.add_argument('--save_folder_name', type=str, default='badnet',
...
def main():
    ...
    from utils.purify_utils import purify, prepare_finetune_dataset, final_purified_dataset_generation
    
    adv_train_ds_purify = deepcopy(adv_train_ds)
    adv_train_ds_purify.ori_image_transform_in_loading = transforms.ToTensor()

    cur_pur_train_ds, sus_train_ds, index_cur_purified, index_suspicious = purify(adv_train_ds_purify, trainer.model, args=args)
    cur_pur_train_ds.ori_image_transform_in_loading = train_img_transform
    adv_train_ds_purify.ori_image_transform_in_loading = train_img_transform

    benign_net = generate_cls_model(
        model_name=args.model,
        num_classes=args.num_classes,
        image_size=args.img_size[0],
    )
    
    benign_trainer = generate_cls_trainer(
        benign_net,
        args.attack,
        args.amp,
    )

    optimizer, scheduler = argparser_opt_scheduler(benign_net, args)

    benign_trainer.train_with_test_each_epoch_v2_sp(
        batch_size=args.batch_size,
        train_dataset=cur_pur_train_ds,
        test_dataset_dict={
            "test_data": benign_test_ds,
            "adv_test_data": adv_test_dataset,
        },
        ...
        frequency_test=1,
        save_folder_path=save_path,
        save_prefix='benign_stage1',
        continue_training_path=None,
    )

    finetune_train_ds = prepare_finetune_dataset(sus_train_ds, cur_pur_train_ds, benign_trainer.model)
    finetune_train_ds.ori_image_transform_in_loading = train_img_transform
    args.lr = args.ft_lr
    optimizer, scheduler = argparser_opt_scheduler(benign_net, args)
    ...
    benign_trainer.train_with_test_each_epoch_v2_sp(
        ...
        frequency_test=1,
        save_folder_path=save_path,
        save_prefix='benign_stage2',
        continue_training_path=None,
    )
    
    final_purified_dataset_generation(adv_train_ds_purify, cur_pur_train_ds, sus_train_ds, index_cur_purified, index_suspicious, benign_trainer.model)
